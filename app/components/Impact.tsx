...
export default function Impact() {
  return (
    <section id="impact" style={{ background: "#e6e8ee", borderTop: "1px solid rgba(21,24,31,.12)" }}>
      <div style={{ padding: "calc(84px*var(--d)) var(--pad-x)", maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 24, marginBottom: 48, flexWrap: "wrap" }}>
          <h2 style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(28px, 6vw, 46px)", lineHeight: 1.05, fontWeight: 400, margin: 0, letterSpacing: "-.01em", maxWidth: 560 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "var(--cols-4)", gap: 0, borderTop: "1px solid rgba(21,24,31,.18)" }}>
          {stats.map((s, i) => (
            <div
              key={s.label}
              style={{
                padding: i === 0 ? "28px 28px 28px 0" : i === stats.length - 1 ? "28px 0 28px 28px" : "28px 28px",
                borderRight: i < stats.length - 1 ? "var(--rule-v)" : "none",
              }}
            >
              <div style={{ fontFamily: "var(--font-serif)", fontSize: "clamp(32px, 7vw, 40px)", fontWeight: 500, marginBottom: 10 }}>{s.big}</div>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6 }}>{s.label}</div>
              <p style={{ fontSize: 13.5, lineHeight: 1.55, color: "#555c6a", margin: 0 }}>{s.body}</p>
            </div>
          ))}
        </div>
      </div>
